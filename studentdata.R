...
# correlation matrix
numeric_cols <- merged_df %>% select_if(is.numeric)
corr_matrix <- cor(numeric_cols, use = "complete.obs")
ggcorrplot(corr_matrix, method = "circle", type = "lower", lab = TRUE, title = "Correlation Matrix of Numeric Variables")

# word cloud of majors
title_freq <- table(merged_df$Title)
wordcloud(names(title_freq), title_freq, colors = brewer.pal(8, "Dark2"), random.order = FALSE, scale = c(2, 0.5))
